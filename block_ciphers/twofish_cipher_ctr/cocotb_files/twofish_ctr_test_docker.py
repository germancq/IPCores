...
async def rst_function_test(dut):
    ...
    if dut.R_i[0] != 0:
        raise TestFailure(
            """Error R0 in rst, wrong_value = {0}""".format(hex(int(dut.R_i[0].value)))
        )
    if dut.R_i[1] != 0:
        raise TestFailure(
            """Error R1 in rst, wrong_value = {0}""".format(hex(int(dut.R_i[1].value)))
        )
    if dut.R_i[2] != 0:
        raise TestFailure(
            """Error R2 in rst, wrong_value = {0}""".format(hex(int(dut.R_i[2].value)))
        )
    if dut.R_i[3] != 0:
        raise TestFailure(
            """Error R3 in rst, wrong_value = {0}""".format(hex(int(dut.R_i[3].value)))
        )
