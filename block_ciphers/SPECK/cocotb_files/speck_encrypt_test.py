...
def setup_dut(dut, plaintext, speck_cipher_sw):
...
async def load_plaintext_test(dut, plaintext, speck_sw):
    # preambule in speck_sw
    y = plaintext & ((2**speck_sw.n) - 1)
    x = (plaintext >> speck_sw.n) & ((2**speck_sw.n) - 1)
    ############################################
...
async def encrypt_loop_test(dut, plaintext, speck_sw):

    i = 0

    y_sw = plaintext & ((2**speck_sw.n) - 1)
    x_sw = (plaintext >> speck_sw.n) & ((2**speck_sw.n) - 1)
...
async def test(dut, index=0):
    ...
    setup_dut(dut, plaintext, speck_cipher_sw)

    await rst_function_test(dut)
    await load_plaintext_test(dut, plaintext, speck_cipher_sw)
    await encrypt_loop_test(dut, plaintext, speck_cipher_sw)

    expected_result = speck_cipher_sw.encrypt(plaintext)
