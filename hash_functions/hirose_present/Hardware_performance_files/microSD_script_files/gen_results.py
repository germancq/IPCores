...
def create_fields(sheet1):
    ...
    sheet1.write(0,3,'ciphertext_100MHz')
    sheet1.write(0,4,'error_100MHz')
    sheet1.write(0,5,'HW_time_ns_100Mhz')
    sheet1.write(0,6,'ciphertext_200MHz')
    sheet1.write(0,7,'error_200MHz')
    sheet1.write(0,8,'HW_time_ns_200Mhz')
    sheet1.write(0,9,'ciphertext_400MHz')
    sheet1.write(0,10,'error_400MHz')
    sheet1.write(0,11,'HW_time_ns_400Mhz')
...
def read_params_from_sd(block_n,micro_sd):
    ...
    result_100 = int.from_bytes(micro_sd.read(16),byteorder='little')
    exec_time_100 = int.from_bytes(micro_sd.read(8),byteorder='little')
    result_200 = int.from_bytes(micro_sd.read(16),byteorder='little')
    exec_time_200 = int.from_bytes(micro_sd.read(8),byteorder='little')
    result_400 = int.from_bytes(micro_sd.read(16),byteorder='little')
...
def write_params(sheet1, params , i):
    ...
    result_200 = params[3]
    result_400 = params[4]
    hw_time_100 = int(calculated_time_in_ns(params[6],100))
    hw_time_200 = int(calculated_time_in_ns(params[7],100))
    hw_time_400 = int(calculated_time_in_ns(params[8],100))
    print('/////////////////////////')
    print(hex(params[2]))
    print(hex(params[3]))
    print(hex(params[4]))
    ...
    if(result_100 != params[5]):
        error_100 = 1

    if(result_200 != params[5]):
        error_200 = 1

    if(result_400 != params[5]):
        error_400 = 1        
    
    sheet1.write(i,1,hex(text))
    sheet1.write(i,2,hex(params[5]))
    sheet1.write(i,3,hex(result_100))
    sheet1.write(i,4,hex(error_100))
    sheet1.write(i,5,hw_time_100)
    sheet1.write(i,6,hex(result_200))
    sheet1.write(i,7,hex(error_200))
    sheet1.write(i,8,hw_time_200)
    sheet1.write(i,9,hex(result_400))
    sheet1.write(i,10,hex(error_400))
    sheet1.write(i,11,hw_time_400)
