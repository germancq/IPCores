...
async def rst_function_test(dut, expected_padded_msg):
    ...
    if dut.padded_msg.value != expected_padded_msg:
        raise TestFailure(
            """Error in padded_msg, wrong value = {0}, expected value = {1}""".format(
                hex(int(dut.padded_msg.value)), hex(expected_padded_msg)
            )
        )
...
async def absorbing_test(dut, expected_state, spongent_impl):
    ...
    if (
        dut.absorbing_phase_impl.permutation_initial_state.value
        != spongent_impl.absorbing_before_p_states[0]
    ):
        raise TestFailure(
            """Error in absorbing initial state, wrong value = {0}, expected value = {1}""".format(
                hex(int(dut.absorbing_phase_impl.permutation_initial_state.value)),
                hex(spongent_impl.absorbing_before_p_states[0]),
            )
        )
    ...
    while dut.end_absorbing.value == 0:
        if dut.absorbing_phase_impl.end_permutation.value == 1:
            ...
            if (
                dut.absorbing_phase_impl.absorbing_state.value
                != spongent_impl.absorbing_after_p_states[i]
            ):
                raise TestFailure(
                    """Error in absorbing after permutation state, wrong value = {0}, expected value = {1} at {2}""".format(
                        hex(int(dut.absorbing_phase_impl.absorbing_state.value)),
                        hex(spongent_impl.absorbing_after_p_states[i]),
                        i,
                    )
                )
    ...
    if dut.absorbing_state.value != expected_state:
        raise TestFailure(
            """Error in absorbing state, wrong value = {0}, expected value = {1}""".format(
                hex(int(dut.absorbing_state.value)), hex(expected_state)
            )
        )
...
async def squeezing_test(dut, spongent_impl, expected_result):
    ...
    if dut.squeezing_phase_impl.result.value != spongent_impl.squeezing_results[0]:
        raise TestFailure(
            """Error in squeezing initial state, wrong value = {0}, expected value = {1}""".format(
                hex(int(dut.squeezing_phase_impl.result.value)),
                hex(spongent_impl.squeezing_results[0]),
            )
        )
    ...
    while dut.squeezing_phase_impl.end_squeezing.value == 0:
        if dut.squeezing_phase_impl.end_permutation.value == 1:
            ...
            if (
                dut.squeezing_phase_impl.state.value
                != spongent_impl.squeezing_states[i]
            ):
                raise TestFailure(
                    """Error in squeezing state, wrong value = {0}, expected value = {1} at {2}""".format(
                        hex(int(dut.squeezing_phase_impl.state.value)),
                        hex(spongent_impl.squeezing_states[i]),
                        i,
                    )
                )

            if (
                dut.squeezing_phase_impl.result.value
                != spongent_impl.squeezing_results[i]
            ):
                raise TestFailure(
                    """Error in squeezing result, wrong value = {0}, expected value = {1} at {2}""".format(
                        hex(int(dut.squeezing_phase_impl.result.value)),
                        hex(spongent_impl.squeezing_results[i]),
                        i,
                    )
                )
